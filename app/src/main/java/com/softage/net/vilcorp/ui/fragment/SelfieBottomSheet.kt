...
class SelfieBottomSheet : BottomSheetDialogFragment() {
    ...
    companion object {
        ...
        fun newInstance(type: String, lat: Double, lng: Double): SelfieBottomSheet {
            val fragment = SelfieBottomSheet()
            val bundle = Bundle()
...
//            dismiss()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lat = arguments?.getDouble(KEY_LAT) ?: 0.0
        lng = arguments?.getDouble(KEY_LNG) ?: 0.0
    ...
    private fun getPreSignedUrls(file_name:String) {
        ...
        val call: Call<PresignedUrlSuccess> = mApiInterface!!.getPreSignedUrl("Bearer ${Utility.token}",JsonHelper.getPreSignedUrl("image/jpeg",file_name,"attendance"))
    ...
    private fun uploadToS3() {
        progressDialog.start("Uploading Selfie...")
            ...
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                ...
                requireActivity().runOnUiThread {
                    ...
                    if (response.code in 200..299) {
                        ...
                        if (type == "punchIn") {
                            getPunchIn(fileUrl, key)
                        } else {
                            getPunchOut(fileUrl, key)
                        }
    ...
    private fun getPunchIn(url: String,key:String) {
        progressDialog.start("Punching In...")
        val call: Call<PunchInSuccess> = mApiInterface!!.punchIN("Bearer ${Utility.token}",JsonHelper.punchIn(url,key,"punchIn",lat,lng))
        call.enqueue(object : Callback<PunchInSuccess?> {
            override fun onResponse(
                ...
                if (response.code() == 200) {
                    assert(response.body() != null)
                    ToastHelper().snackBar(requireActivity().findViewById(android.R.id.content),"Punch In successfully")
                    mUtil!!.setDuty(true)
                    mUtil!!.setAttendanceMarked(true)
                    val intent = Intent(requireContext(), MainActivity::class.java)
    ...
    private fun getPunchOut(url: String,key:String) {
        progressDialog.start("Punching Out...")
        val call: Call<PunchInSuccess> = mApiInterface!!.punchOut("Bearer ${Utility.token}",JsonHelper.punchIn(url,key,"punchOut",lat,lng))
        call.enqueue(object : Callback<PunchInSuccess?> {
            override fun onResponse(
                call: Call<PunchInSuccess?>,
                response: Response<PunchInSuccess?>
                ...
                if (response.code() == 200) {
                    assert(response.body() != null)
                    ToastHelper().snackBar(requireActivity().findViewById(android.R.id.content),"Punch Out successfully")
                    mUtil!!.setDuty(false)
                    val intent = Intent(requireContext(), MainActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
                    startActivity(intent)
